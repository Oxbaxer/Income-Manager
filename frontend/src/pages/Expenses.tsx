...
export function ExpensesPage() {
  ...
  async function selectAllFiltered() {
    ...
    try {
      ...
      const res = await api.get<{ ids: number[] }>(`/api/expenses/all-ids${qs ? `?${qs}` : ''}`)
  ...
  async function applyRecategorize() {
    ...
    try {
      await api.post('/api/expenses/bulk-update', {
  ...
  useEffect(() => {
    ...
    if (filterCategoryId) params.set('categoryId', filterCategoryId)
    Promise.all([
      api.get<PaginatedResponse<ExpenseTransaction>>(`/api/expenses?${params.toString()}`),
  ...
  return (
    <PageShell
      ...
      action={<Button onClick={openCreate} size="sm">+ {t('expenses.add')}</Button>}
    >
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <p className="text-sm text-white/50 flex-1">{total} {total > 1 ? 'transactions' : 'transaction'}</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-right px-4 py-3 text-xs font-medium text-white/40 uppercase tracking-wider">{t('expenses.amount')}</th>
            ...
            <tbody>
              {transactions.length === 0 ? (
                ...
                <tr key={tx.id} className={`border-b border-border/50 hover:bg-white/2 transition-colors ${selectedIds.has(tx.id) ? 'bg-primary/5' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    <span
                      ...
                      style={{
                        ...
                        border: `1px solid ${tx.categoryColor ?? '#6366f1'}44`,
                      }}
                    >
                  ...
                  <td className="px-4 py-3 text-right font-mono font-semibold text-accent-red">
      ...
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); setEditQueue([]) }} title={editQueue.length > 1 ? `${t('expenses.edit')} (${editQueue.length} restantes)` : (editId ? t('expenses.edit') : t('expenses.add'))}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Select label={t('expenses.category')} error={errors.categoryId?.message} {...register('categoryId')}>
            <option value="">—</option>
            {categories.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
          </Select>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-white/70">
            Affecter une nouvelle catégorie à <strong className="text-white">{selectedIds.size}</strong> dépense{selectedIds.size > 1 ? 's' : ''}.
          </p>
          <Select
